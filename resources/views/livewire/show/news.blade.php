...
mount(function ($slug) {
    ...
    // Set SEO metadata
    SEOMeta::setTitle($this->news->title);
    SEOMeta::setDescription(Str::limit(strip_tags($this->news->description), 170, '...'));
    SEOMeta::setCanonical(request()->url());

    OpenGraph::setTitle($this->news->title);
    OpenGraph::setDescription(Str::limit(strip_tags($this->news->description), 170, '...'));
    OpenGraph::setUrl(request()->url());
    OpenGraph::addProperty('type', 'webpage');

    SEOMeta::addMeta('twitter:card', 'summary_large_image');
    SEOMeta::addMeta('twitter:title', $this->news->title);
    SEOMeta::addMeta('twitter:description', Str::limit(strip_tags($this->news->description), 170, '...'));
...
<div class="relative container mx-auto p-4 sm:p-6 mt-12">
        ...
        <div class="absolute rounded-3xl glass top-0 left-0 w-full h-[70vh] sm:h-[60vh] bg-gradient-to-b from-amber-500 to-transparent z-10 pointer-events-none shadow-none"></div>
        ...
        <article class="prose prose-lg max-w-3xl mx-auto relative z-20 {{ in_array(app()->getLocale(), ['fa', 'ar']) ? 'rtl' : 'ltr' }}">
            ...
            <div class="flex items-center justify-between text-sm text-white mb-6 drop-shadow-sm">
                <div class="flex items-center gap-2">
                    @if($news->newsSite)
                        <span class="font-medium">{{ $news->newsSite->name }} |</span>
                    @endif
                </div>
            ...
            <div class="content text-justify leading-relaxed text-gray-700 text-lg">
                ...
                @if($news->source_url)
                    <a href="{{ $news->source_url }}" target="_blank" rel="noopener noreferrer" class="btn btn-soft btn-accent btn-block my-4 ">
